...
describe('NotificationTemplateService (property-based)', () => {
  ...
  describe('formatEntryTriggered', () => {
    ...
    it('should contain correct emoji for each side', () => {
      fc.assert(
        fc.property(
          fc.constantFrom(TradeSide.LONG, TradeSide.SHORT, TradeSide.SPOT),
          (side) => {
            ...
            const result = service.formatEntryTriggered(trade);
            const expectedEmoji = side === TradeSide.LONG ? '🟢' : side === TradeSide.SHORT ? '🔴' : '⚪';
            return result.includes(expectedEmoji);
          }
        ),
        { numRuns: 100 }
      );
  ...
  describe('formatTPHit', () => {
    ...
    it('should format RR with 2 decimal places', () => {
      fc.assert(
        fc.property(
          ...
          (rr) => {
            ...
            return result.includes(`${rr.toFixed(2)}R`);
          }
        ),
        { numRuns: 200 }
      );
